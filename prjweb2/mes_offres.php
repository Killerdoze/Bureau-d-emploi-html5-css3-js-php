...
$codes = array();
$Id_employeur = null;
$req1 = $conn->prepare("SELECT * FROM employeur WHERE Pseudo = :pseudo ");
$req1->bindValue(':pseudo', $pseudo);
$req1->execute();
$resultats1 = $req1->fetchAll(PDO::FETCH_ASSOC);
if (isset($resultats1[0]['ID'])) {
    $Id_employeur = $resultats1[0]['ID'];
}
if ($Id_employeur != null) {
    $req12 = $conn->prepare("SELECT * FROM employeur_offre WHERE ID_offreur = :Id_employeur");
    $req12->bindValue(':Id_employeur', $Id_employeur);
    $req12->execute();
    $resultats12 = $req12->fetchAll(PDO::FETCH_ASSOC);

    foreach ($resultats12 as $resultat) {
        $codes[] = $resultat['code_offre_emploi'];
    }
}

if (!empty($codes)) {
    $req2 = "SELECT * FROM offre_emploi WHERE Code_offre_emploi IN (" . implode(',', array_fill(0, count($codes), '?')) . ")";
...
if (empty($resultats2)) {
    echo "<script>alert('Offre est supprimé avec succès');</script>";
}
      ...
      <div class="card horizontal">
        ...
        <div class="card-stacked">
          <div class="card-content">
            ...
            <p><?php echo $resultat['Description']; ?> Salaire s'eleve à : <?php echo $resultat['Salaire_propose']; ?></p><br>
			...
			<a href="acceuilemployeur.php?code=<?php echo $resultat['Code_offre_emploi']; ?>" class="btn btn-styled" target="_blank" onclick="alert('Offre est supprimé avec succeés');">Cliquez ici pour Supprimer l'offre</a>
